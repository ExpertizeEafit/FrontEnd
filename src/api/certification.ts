...
export const getSeniorityRequests = ():Promise<SeniorityRequestStatus[]>  => {
    ...
    const userData = JSON.parse(getCookie("user") || "{}")
    const { token } = userData
    return axios.get(`http://localhost:80/certifications/seniority`, 
    {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
...
export const createSeniorityRequestStatus = (data:any):Promise<any>  => {
    ...
    const userData = JSON.parse(getCookie("user") || "{}")
    const { token } = userData
    return axios.post(`http://localhost:80/certifications/seniority`, data,
    {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
...
export const updateSeniorityRequestStatus = (data:any):Promise<any>  => {
    ...
    const userData = JSON.parse(getCookie("user") || "{}")
    const { token } = userData
    return axios.put(`http://localhost:80/certifications/seniority`, data,
    {
        headers: {
            'Content-Type': 'application/json',
            'Authorization': `Bearer ${token}`
